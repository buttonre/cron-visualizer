...
const API_URL   = "http://YOUR_LINUX_IP:8765";  // e.g. http://192.168.1.50:8765
...
// ─── CRON → UI STATE PARSER ───────────────────────────────────────────────────

function cronToState(expr) {
  if (!expr) return defaultState();
  const parts = expr.trim().split(/\s+/);
  ...
  const toAmPm = (h24) => {
    const h = h24 === 0 ? 12 : h24 > 12 ? h24 - 12 : h24;
    return { hour: h, ampm: parseInt(h24) < 12 ? "AM" : "PM" };
  };
  const snapMinute = (m) => String(Math.round(m / 5) * 5).padStart(2, "0");
  ...
  if (dom==="*" && month==="*" && dow==="*" && !min.includes("*") && !hour.includes("*")) {
    ...
    return { freq:"daily", interval:30, atMinute:"0", hour: h, minute: snapMinute(parseInt(min)), ampm, days:[], dom:1 };
  }
  if (dom==="*" && month==="*" && !dow.includes("*") && !min.includes("*") && !hour.includes("*")) {
    const { hour: h, ampm } = toAmPm(parseInt(hour));
    return { freq:"weekly", interval:30, atMinute:"0", hour: h, minute: snapMinute(parseInt(min)), ampm, days: dow.split(",").map(Number), dom:1 };
  }
  if (!dom.includes("*") && month==="*" && dow==="*" && !min.includes("*") && !hour.includes("*")) {
    const { hour: h, ampm } = toAmPm(parseInt(hour));
    return { freq:"monthly", interval:30, atMinute:"0", hour: h, minute: snapMinute(parseInt(min)), ampm, days:[], dom: parseInt(dom) };
  }
  return defaultState();
}
...
const STATUS_CONFIG = {
  ...
  never:  { label:"NEVER RUN", color:"#4b5563", pulse:false },
...
function SchedulePicker({ cronExpression, onSave, onCancel }) {
  ...
  return (
    <div style={{
      background:"rgba(10,14,23,0.98)", border:"1px solid rgba(34,197,94,0.3)",
      borderRadius:8, padding:"14px 14px 12px", marginTop:6,
      ...
      {state.freq === "weekly" && (
        <div style={{ marginBottom:10 }}>
            ...
            {DAY_NAMES.map((name, i) => {
              ...
              return (
                <button key={i} onClick={() => toggleDay(i)} style={{
                  ...
                  border:"1px solid " + (active ? "#22c55e88" : "rgba(255,255,255,0.1)"),
          ...
          {state.days.length === 0 && (
            <div style={{ fontSize:9, color:"#ef4444", marginTop:4 }}>Select at least one day</div>
          )}
        </div>
      )}
      ...
      {["daily","weekly","monthly"].includes(state.freq) && (
        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10 }}>
          ...
          <select value={state.hour} onChange={e => set("hour", parseInt(e.target.value))} style={selectStyle}>
            {HOURS_12.map(h => <option key={h} value={h}>{h}</option>)}
          </select>
          <span style={{ color:"#4b5563", fontSize:12, fontWeight:700 }}>:</span>
          <select value={state.minute} onChange={e => set("minute", e.target.value)} style={selectStyle}>
            {MINUTES.map(m => <option key={m} value={m}>{m}</option>)}
          </select>
          <select value={state.ampm} onChange={e => set("ampm", e.target.value)} style={selectStyle}>
      ...
      <div style={{ display:"flex", gap:8 }}>
        <button onClick={() => canSave && onSave(cron)} disabled={!canSave} style={btnStyle("#22c55e", !canSave)}>
          ✓ SAVE
        </button>
  ...
  return (
    <div style={{ marginBottom: open ? 4 : 8 }}>
      ...
      >
        ...
        <span style={{ fontSize:11, fontWeight:600, color:"#cbd5e1", borderBottom:"1px dotted #374151" }}>
      ...
      {open && (
        <SchedulePicker
          ...
          onSave={cron => { onSave(cron); setOpen(false); }}
...
function EditableField({ value, onSave, style }) {
  ...
  if (!editing) {
    return (
      <span style={{ ...style, display:"inline-flex", alignItems:"center", gap:4, cursor:"text" }}
        onClick={() => { setDraft(value); setEditing(true); }} title="Click to edit">
        {value}
        <span style={{ fontSize:9, color:"#374151", opacity:0.7 }}>✎</span>
      </span>
    );
  }
  return (
    <span style={{ display:"inline-flex", alignItems:"center", gap:4 }}>
      <input ref={inputRef} value={draft}
        ...
        style={{
          background:"rgba(255,255,255,0.06)", border:"1px solid #22c55e55",
          borderRadius:4, color:"#e2e8f0", fontSize:style?.fontSize||12,
          fontFamily:"inherit", fontWeight:style?.fontWeight||"normal",
          padding:"2px 6px", outline:"none", width:200,
        }}
      />
  ...
  return (
    <div onClick={onChange} title={enabled ? "Click to disable" : "Click to enable"}
      style={{
        ...
        border:"1px solid " + (enabled ? "#16a34a" : "#4b5563"),
      }}>
      <div style={{
...
function TaskCard({ task, onToggle, onEdit }) {
  ...
  return (
    <div style={{
      ...
      border:"1px solid " + (task.enabled ? "rgba(255,255,255,0.09)" : "rgba(255,255,255,0.04)"),
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ fontSize:12, fontWeight:800, color:"#e2e8f0" }}>
            <EditableField
              ...
              style={{ fontSize:12, fontWeight:800, color:"#e2e8f0" }}
            />
          </div>
          <div style={{ fontSize:9, color:"#475569", marginTop:2, fontFamily:"monospace" }}>{task.command}</div>
        </div>
        <div style={{
          fontSize:8, fontWeight:700, padding:"2px 8px", borderRadius:10, whiteSpace:"nowrap",
      ...
      <ScheduleEditor
        ...
        onSave={cron => onEdit(task.taskId, { cronExpression: cron })}
      />
        ...
        {[
          ...
          <div key={label} style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:6, padding:"6px 8px" }}>
            ...
            <div style={{ fontSize:11, color, fontWeight:700 }}>
              {value || <span style={{ color:"#374151" }}>N/A</span>}
            </div>
          </div>
        ))}
      </div>
...
export default function CronVisualizer() {
  ...
  const handleToggle = useCallback(async (id) => {
    ...
    try {
      const res = await fetch(API_URL + "/crons/toggle", {
        method: "POST", headers: API_HEADERS,
        body: JSON.stringify({ index: task.index }),
      });
      if (!res.ok) throw new Error("toggle failed");
    } catch {
      setTasks(prev => prev.map(t => t.taskId === id ? {...t, enabled: task.enabled} : t));
    }
  }, [tasks]);
  ...
  const handleEdit = useCallback(async (id, changes) => {
    ...
    if (changes.cronExpression) {
      try {
        const res = await fetch(API_URL + "/crons/update", {
          method: "POST", headers: API_HEADERS,
          body: JSON.stringify({ index: task.index, cronExpression: changes.cronExpression }),
        });
        if (!res.ok) throw new Error("update failed");
      } catch {
        setTasks(prev => prev.map(t => t.taskId === id ? {...t, cronExpression: task.cronExpression} : t));
      }
    }
  }, [tasks]);
  ...
  return (
    <div style={{
      ...
      background:"#0a0e17", color:"#e2e8f0", minHeight:"100vh", padding:"12px", fontSize:12,
    }}>

      {/* Header */}
      <div style={{
        display:"flex", alignItems:"center", gap:10, marginBottom:14,
        borderBottom:"1px solid rgba(34,197,94,0.25)", paddingBottom:10,
      }}>
        ...
        <span style={{ fontSize:9, fontWeight:700, padding:"2px 8px", borderRadius:4, background:"#22c55e22", color:"#22c55e", border:"1px solid #22c55e44" }}>
          {activeTasks} ACTIVE
        </span>
        {pausedTasks > 0 && (
          <span style={{ fontSize:9, fontWeight:700, padding:"2px 8px", borderRadius:4, background:"#f59e0b22", color:"#f59e0b", border:"1px solid #f59e0b44" }}>
            {pausedTasks} PAUSED
          </span>
        )}
        <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:10 }}>
          ...
          <button onClick={handleRefresh} disabled={refreshing} style={{
            ...
            border:"1px solid rgba(255,255,255,0.1)", color: refreshing ? "#4b5563" : "#94a3b8",
            cursor: refreshing ? "wait" : "pointer", letterSpacing:0.5,
          }}>
      ...
      {error && (
        <div style={{
          ...
          <span style={{ fontSize:9, color:"#94a3b8", marginLeft:"auto" }}>
            Check API_URL in CronVisualizer.jsx · Is cron_api.py running?
          </span>
        </div>
      )}
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {loading ? (
          ...
          <div style={{ textAlign:"center", padding:"40px 20px", background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:10, color:"#374151" }}>
            ...
            <div style={{ fontSize:10, marginTop:4 }}>Run crontab -e on the server to add one</div>
          </div>
        ) : tasks.map(task => (
          <TaskCard key={task.taskId} task={task} onToggle={handleToggle} onEdit={handleEdit} />
        ))}
      </div>
      ...
      <div style={{ marginTop:14, paddingTop:10, borderTop:"1px solid rgba(255,255,255,0.05)", display:"flex", justifyContent:"space-between", fontSize:9, color:"#1e293b" }}>
        <span>CRON VISUALIZER · SPRINT 4</span>
